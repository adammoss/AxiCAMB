...
def run_test(m_ax=1e-25, ax_fraction=0.0, lmax=3000, halofit_version='mead2020', accuracy_boost=1,
    ...
    if ax_fraction > 0 and k_ax is not None:
        from scipy.interpolate import interp1d as interp1d_hm
        interp_hm = interp1d_hm(k_ax, pk_nl_axion_hm[z0_idx, :], kind='linear', bounds_error=False, fill_value=np.nan)
        pk_hm_interp = interp_hm(k_h)
        valid_hm = np.isfinite(pk_hm_interp) & valid_k
    else:
        pk_hm_interp = pk_nl_axion_hm[z0_idx, :]
        valid_hm = valid_k
